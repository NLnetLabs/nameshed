...
impl LoaderState {
        ...
        let ongoing = match refresh {
            EnqueuedRefresh::Refresh => {
                ...
                let handle = tokio::task::spawn(Self::refresh(
                    zone, start, source, latest, loader,
                ));
                OngoingRefresh::Refresh { start, handle }
            }
            EnqueuedRefresh::Reload => {
                let handle = tokio::task::spawn(Self::reload(
                    zone, start, source, loader,
                ));
    ...
    async fn reload(
...

        {
            ...
            // Update the zone refresh timers.
            let soa =
                state.contents.as_ref().map(|contents| &contents.latest.soa);
            if result.is_ok() {
                state.loader.refresh_timer.schedule_refresh(
                    &zone,
                    start,
            ...
            update = match result {
                ...
                Ok(Some(serial)) => {
                    ...
                    let mut parser = octseq::Parser::from_ref(&bytes);
                    let zone_name =
                        domain::base::Name::parse(&mut parser).unwrap();
                    let zone_serial = domain::base::Serial(serial.into());
