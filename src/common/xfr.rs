...
pub fn parse_xfr_acl(
    ...
    let parts: Vec<String> =
        xfr_in.split(" KEY ").map(ToString::to_string).collect();
    ...
    let addr = SocketAddr::from_str(addr)
        ...
        .inspect_err(|err| {
            error!("Error: Invalid XFR ACL address '{addr}': {err}")
        })
        .map_err(|_| ExitError)?;
    ...
    if let Some(key_name) = key_name {
        let encoded_key_name = KeyName::from_str(key_name)
            .inspect_err(|err| {
                error!("Error: Invalid TSIG key name '{key_name}': {err}")
            })
            .map_err(|_| ExitError)?;
        if let Some(key) = key_store.get_key_by_name(&encoded_key_name) {
            xfr_cfg.tsig_key =
                Some((encoded_key_name.clone(), key.algorithm()));
